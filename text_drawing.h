...
namespace uih {
        ...
        void analyse(HDC dc, const wchar_t* p_str, size_t p_str_len, int max_cx, bool b_clip)
        {
            ...
            if (m_string_length)
                ScriptStringAnalyse(dc, p_str, p_str_len, NULL, -1,
                                    SSA_FALLBACK | SSA_GLYPHS | SSA_LINK | (b_clip ? SSA_CLIP : NULL), max_cx, &m_sc,
                                    &m_ss, nullptr, nullptr, nullptr, &m_ssa);
        }
    ...
    BOOL text_out_colours_ellipsis(HDC dc, const char* src, int len, int x_offset, int pos_y, const RECT* base_clip,
                                   ...
                                   int* p_position = nullptr);
    BOOL text_out_colours_tab(HDC dc, const char* display, int display_len, int left_offset, int border,
                              const RECT* base_clip, bool selected, DWORD default_color, bool columns, bool tab,
                              bool show_ellipsis, alignment align, unsigned* p_width = nullptr,
                              bool b_set_default_colours = true, bool b_vertical_align_centre = true,
                              int* p_position = nullptr);
