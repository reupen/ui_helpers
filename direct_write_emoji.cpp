...
public:
    explicit EmojiFontFallback(icu::ICUHandle::Ptr icu_handle,
        const wil::com_ptr<IDWriteFontCollection>& font_collection, wil::com_ptr<IDWriteFontFallback> base_fallback,
        const wchar_t* emoji_family_name, const wchar_t* monochrome_family_name)
        : m_base_fallback(std::move(base_fallback))
        , m_icu_handle(std::move(icu_handle))
    {
...
private:
        ...
        std::optional<EmojiType> previous_character_type = {}) const
    {
        ...
        const auto is_emoji = m_icu_handle->u_has_binary_property(static_cast<int32_t>(decoded), UCHAR_EMOJI);
        const auto is_emoji_presentation
            = m_icu_handle->u_has_binary_property(static_cast<int32_t>(decoded), UCHAR_EMOJI_PRESENTATION);
        const auto is_emoji_component
            = m_icu_handle->u_has_binary_property(static_cast<int32_t>(decoded), UCHAR_EMOJI_COMPONENT);
        const auto has_variation = emoji::emojis_with_variations.contains(decoded);
...
wil::com_ptr<IDWriteFontFallback> create_emoji_font_fallback(icu::ICUHandle::Ptr icu_handle,
    const wil::com_ptr<IDWriteFontCollection>& font_collection, wil::com_ptr<IDWriteFontFallback> base_fallback,
    const wchar_t* emoji_family_name, const wchar_t* monochrome_family_name)
{
    return new EmojiFontFallback(
        std::move(icu_handle), font_collection, std::move(base_fallback), emoji_family_name, monochrome_family_name);
}
