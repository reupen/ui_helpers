...
void TextLayout::render(HDC dc, RECT rect, COLORREF default_colour, float x_origin_offset, float y_origin_offset) const
{
    ...
    const auto draw_top_px
        = std::max(0l, gsl::narrow_cast<long>((y_origin_offset - overhang_metrics.top) * scaling_factor) - 1);
    const auto draw_bottom_px = std::min(rect_height,
        gsl::narrow_cast<long>((y_origin_offset + layout_height + overhang_metrics.bottom) * scaling_factor + 1.0f)
            + 1);
    ...
    THROW_IF_FAILED(
        m_text_layout->Draw(NULL, renderer.get(), -draw_left_dip + x_origin_offset, -draw_top_dip + y_origin_offset));

    BitBlt(dc, source_x, source_y, bitmap_width, bitmap_height, memory_dc, 0, 0, SRCCOPY);
}

void TextLayout::render_with_background(HDC dc, RECT output_rect, RECT clip_rect, COLORREF background_colour,
    COLORREF default_text_colour, float x_origin_offset, float y_origin_offset) const
{
    ...
    THROW_IF_FAILED(m_text_layout->Draw(NULL, renderer.get(), x_origin_offset, y_origin_offset));
