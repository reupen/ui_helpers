...
void InfoBox::create(HWND wnd_parent, const char* p_title, const char* p_text, INT oem_icon)
{
    ...
    const auto fixed_text = std::regex_replace(p_text, std::regex("\n"), "\r\n");

    SetWindowText(m_wnd_edit, mmh::to_utf16(fixed_text).c_str());
    ...
    const int cy = std::min(calc_height(), std::max(static_cast<int>(wil::rect_height(rc)), scale_dpi_value(150)));

    const int y = std::max<int>(rc.top + (wil::rect_height(rc) - cy) / 2, rc.top);

    SetWindowPos(wnd, nullptr, (rc.left + (wil::rect_width(rc) - cx) / 2), y, cx, cy, SWP_NOZORDER);
