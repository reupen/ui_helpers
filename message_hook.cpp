...
namespace message_hook_manager
{
    ...
    void register_hook(t_message_hook_type p_type, message_hook * p_hook)
    {
        ...
        {
            g_hooks[p_type].m_hook = SetWindowsHookEx(g_hook_ids[p_type], g_hook_procs[p_type], p_type != type_mouse_low_level ? nullptr : mmh::get_current_instance(), p_type != type_mouse_low_level ? GetCurrentThreadId() : NULL);
        }
    }
