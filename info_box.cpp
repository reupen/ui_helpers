...
void InfoBox::create(HWND wnd_parent, const char* p_title, const char* p_text, INT oem_icon)
{
    ...
    HWND wnd = m_container_window->create(
        wnd_parent, uih::WindowPosition((rc.left + (RECT_CX(rc) - cx) / 2), (rc.top + (RECT_CY(rc) - cy) / 2), cx, cy));
...
LRESULT InfoBox::on_message(HWND wnd, UINT msg, WPARAM wp, LPARAM lp)
{
    switch (msg) {
    case WM_NCCREATE:
        if (m_on_creation)
