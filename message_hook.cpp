...
namespace uih
{
    ...
    {        
        ...
        if (code >= 0)
        {
            ...
            for (n=0; n<count; n++)
            {
                if ((p_type != MessageHookType::type_get_message || wp==PM_REMOVE) && g_hooks[type_index].m_hooks[n]->on_hooked_message(p_type, code, wp, lp))
                {
    ...
    void register_message_hook(MessageHookType p_type, MessageHook * p_hook)
    {
        ...
        {
            g_hooks[type_index].m_hook = SetWindowsHookEx(
                g_hook_ids[type_index], 
                g_hook_procs[type_index],
